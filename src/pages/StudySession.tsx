...
export const StudySession = ({
  ...
  const progress =
    ((currentIndex + (answers[currentIndex] !== null ? 1 : 0)) /
      flashcards.length) *
    100;
  ...
  const getScoreMessage = () => {
    ...
    if (percentage >= 90)
      return { message: "Excellent! 🏆", color: "text-secondary" };
    if (percentage >= 75)
      return { message: "Great job! 🎉", color: "text-primary" };
    if (percentage >= 60)
      return { message: "Good work! 👍", color: "text-accent" };
    ...
    return (
      <div className="max-w-2xl mx-auto text-center space-y-6">
        ...
        <div className="flex gap-4">
          <Button onClick={handleComplete} className="study-button flex-1">
            Continue Learning
          </Button>
          <Button
            variant="outline"
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <Badge className="subject-badge">{currentCard.subject}</Badge>
            </div>
          ...
          <div className="flex justify-between mt-2 text-sm text-muted-foreground">
            ...
            <span>
              Score: {correctAnswers}/{answers.filter((a) => a !== null).length}
            </span>
          </div>
        </CardContent>
      </Card>
